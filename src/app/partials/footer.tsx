    ...
    <div className="max-w-[60rem] w-full m-auto h-64 py-8 gap-8 flex flex-col border-t border-gray-200 ">
      <div className="w-full h-full gap-16 flex flex-row items-center justify-between">
        ...
        <div className="h-full w-72 flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2">
            <textarea className="w-full h-full bg-stone-100 min-h-18 p-2 text-sm h-full resize-none" placeholder="Masukan Anda sangat berarti bagi kami." maxLength={350}/>
            <button className="w-fit text-sm text-stone-900 underline hover:text-stone-400 cursor-pointer">Kirim</button>
          </div>
        </div>
      </div>

      <p className="text-center text-sm text-stone-400">
        &copy; Opinion Journey {new Date().getFullYear()}
      </p>
    </div>
  );
};
